class Vehicle {
  ...
  constructor(x, y, image, pathColor = null) {
    ...
    // Couleur personnalisée (si fournie)
    this.vehicleColor = pathColor || color(255, 255, 255);
    ...
    // trainée derrière les véhicules (limitée)
    this.path = [];
    this.pathLength = 30;
    this.pathSpacingInFrames = 3;
    this.pathColor = pathColor || color(255, 255, 255);
  }
  ...
  show() {
    // dessin du chemin avec dégradé de la couleur du véhicule
    this.path.forEach((p, index) => {
        let alpha = map(index, 0, this.path.length, 50, 255);
        let c = this.vehicleColor;
        stroke(red(c), green(c), blue(c), alpha);
        fill(red(c), green(c), blue(c), alpha);
        let size = map(index, 0, this.path.length, 1, 4);
        circle(p.x, p.y, size);
    });
    ...
    // Teinte de l'image avec la couleur du véhicule
    tint(this.vehicleColor);
